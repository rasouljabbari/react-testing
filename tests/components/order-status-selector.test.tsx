...
describe("OrderStatusSelector", () => {
    ...
    // shoul write this test seprate because it is a default value and can not set parameterical value
    it("should call onChange with 'new' when the New option is selected", async () => {
        const { trigger, user, onChange, getOption } = renderComponent();
        await user.click(trigger); // user click on the combobox

        const processedOption = await getOption(/processed/i);
        await user.click(processedOption); // user click on the processed option

        await user.click(trigger); // user click on the combobox

        const newOption = await getOption(/new/i); // user click on the new option
